...
def my_spark_core_model(sc, my_dataset_dir):
    ...
    # print(mappedRDD.collect())
    print(mappedRDD.collect())
    # reducedRDD = mappedRDD.reduceByKey(lambda x,y: max(x[1][1],y[1][1]))
    # reducedRDD = reducedRDD.map(lambda row: (row[0], row[1], row[2]))

    # groupedRDD = mappedRDD.groupByKey().max()
    reduced = mappedRDD.reduceByKey(lambda x,y : x if (x[1] > y[1]) else y)

    # mapped = groupedRDD.map(lambda row:("%s_%s" % (row[0], row[2]), (row[1], int(row[3]))) )
