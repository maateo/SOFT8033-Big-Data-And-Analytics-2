...
def my_model(ssc, monitoring_dir, window_duration, sliding_duration, time_step_interval):
    ...
    # mappedDStream.pprint()

    mappedDStream = mappedDStream.map(lambda row: ("%s_%s" % (row[0], row[2]), (row[1], int(row[3]))))

    # mappedDStream.pprint()

    windowDStream = mappedDStream.window(60,
                                         60
                                         )

    # windowDStream.pprint()

    # reducedRDD = windowDStream.reduceByKey(lambda x, y: x if (x[1] > y[1]) else y)
...
if __name__ == '__main__':
    ...
    valid_files = ["bikeMon_20170501.csv","bikeMon_20170502.csv"
                   ]
    ...
    source_dir = "FileStore/tables/3_Assignment/matfiles/"
